...
def find_moves(warehouses, order):
    ...
    def helper(current_wh, dest_wh=None):
        ...
        # we could have another block of code to check if originating warehouse has enough for our demand
        # but lets just write the code once instead, passing an optional dest
        # if there is no dest warehouse we dont append anything
            if dest_wh is not None and current_wh != dest_wh:
        ...
        for tup in warehouses[current_wh]["sources"]:
            ...
            helper(src_wh, dest_wh=current_wh)
